...
def _gdn_attention_core_xpu_impl(
    ...
    # XPU-MTP FIX: the fused SYCL gdn_attention kernel asserts
    # spec_query_start_loc has size [num_spec_decodes+1], but the FULL cudagraph
    # path (gdn_attn.py build()) pads this buffer to [num_actual_tokens+1].
    # Slice to the valid prefix (a view into the same persistent buffer, so it
    # stays cudagraph-safe; the padding tail is unused by the kernel).
    if spec_query_start_loc is not None:
        spec_query_start_loc = spec_query_start_loc[: num_spec_decodes + 1]
    spec_token_indx = attn_metadata.spec_token_indx
    spec_state_indices_tensor = attn_metadata.spec_state_indices_tensor  # noqa: E501
    # XPU-MTP FIX (cont.): spec_state_indices_tensor and num_accepted_tokens are
    # also padded to batch_size by the FULL cudagraph path; narrow both to the real
    # num_spec_decodes prefix so the kernel's strict == size checks pass. Narrowing
    # dim 0 of a contiguous tensor yields a contiguous view sharing storage, so this
    # stays cudagraph-safe; the compute iterates by num_spec_decodes (tail unused).
    ...
    import os as _os
    if _os.environ.get("VLLM_GDN_SPEC_DEBUG", "0") == "1" and 0 < num_spec_decodes <= 4:
        _c = getattr(_gdn_attention_core_xpu_impl, "_dbgc", 0)
        if _c < 40:
            _gdn_attention_core_xpu_impl._dbgc = _c + 1
            import sys as _sys
            _si = spec_state_indices_tensor
            _na = num_accepted_tokens
            print(f"[GDNSPEC] call{_c} L={self.prefix} nspec={num_spec_decodes} "
                  f"nacc={_na.detach().cpu().tolist() if _na is not None else None} "
                  f"state_idx={_si.detach().cpu().tolist() if _si is not None else None}",
                  file=_sys.stderr, flush=True)

    _gt0 = __import__('time').perf_counter()
    ...
    if __import__('os').environ.get('VLLM_KPROF'):
        torch.xpu.synchronize()
        _gus = (__import__('time').perf_counter() - _gt0) * 1e6
        _n = globals().setdefault('_KTN', {})
        if num_actual_tokens <= 8: _n['gdn'] = _n.get('gdn', 0) + 1
        if num_actual_tokens <= 8 and _n['gdn'] <= 120:
            __import__('sys').stderr.write(f"[KTIME2] gdn ntok={num_actual_tokens} t={_gus:.0f}us\n")
