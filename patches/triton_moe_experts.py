...
class TritonExperts(LoRAExpertsMixin, mk.FusedMoEExpertsModular):
    ...
    def _supports_quant_scheme(
        ...
        # INT8 requires at least 7.5 (Turing) on CUDA; Intel Xe2 (Battlemage)
        # has native s8xs8->s32 DPAS, so int8 is supported on XPU too.
        # [xpu-graph-work] added `or is_xpu()` to unblock the native int8 MoE
        # oracle on XPU (TritonExperts int8 tl.dot lowers to DPAS on Xe2).
    ...
    def apply(
        ...
        assert hidden_states.dtype in [
            ...
            # [xpu-graph-work] W8A8: the non-EP prepare step pre-quantizes the
            # activation to int8 (with a1q_scale), exactly as it does fp8. The
            # downstream use_int8_w8a8 kernel handles int8 input; allow it.
            torch.int8,
        ]
        ...
        if hidden_states.dtype == torch.bfloat16:
            ...
            # [xpu-graph-work] W8A8: int8 activations, int32 DPAS accumulate,
            # dequant to bf16 output (same output compute_type as the fp8 path).
            compute_type = tl.bfloat16
        else:
