...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # DEBUG probe (env VLLM_SPEC_PROBE): persistent device buffers filled by
        # CAPTURED copies inside _run_model, so on REPLAY they hold what the
        # captured graph actually reads/produces; dumped host-side after
        # run_fullgraph in propose(). Localizes whether the FULL-capture
        # corruption is stale inputs vs corrupted compute.
        import os as _osprobe

        if _osprobe.environ.get("VLLM_SPEC_PROBE"):
            self._probe_pos = torch.zeros(8, dtype=torch.int64, device=device)
            self._probe_ids = torch.zeros(8, dtype=torch.int64, device=device)
            self._probe_hid = torch.zeros(8, dtype=torch.float32, device=device)
            self._probe_out = torch.zeros(8, dtype=torch.float32, device=device)
            self._probe_n = 0
        else:
    ...
    def init_cudagraph_manager(self, cudagraph_mode: CUDAGraphMode) -> None:
        # ISOLATION PROBE: force the drafter (speculator) eager while the
        # target keeps its own cudagraph, to localize drafter-capture vs
        # target-capture as the cause of the low MTP acceptance.
        import os as _osspec

        if _osspec.environ.get("VLLM_SPEC_EAGER"):
    ...
    def capture(
        ...
        # XPU MTP capture fix (mirrors the V1 proposer fix in
        # llm_base_proposer.py dummy_run): the DeepSeek-V4 MTP layer masks
        # inputs_embeds where positions == 0 (fused_mtp_input_rmsnorm:
        # keep = pos != 0). last_token_indices was just zeroed, so _prefill
        # reads positions[0] == 0 at capture -> the mask bakes all-True ->
        # inputs_embeds (token identity) zeroed on every replay -> ~40%
        # draft acceptance. Capture with position 1 so the mask bakes
        # all-False (correct for decode, positions >= 1); restore after.
        _saved_positions = self.input_buffers.positions.clone()
        self.input_buffers.positions.fill_(1)
        try:
            ...
            self.input_buffers.positions.copy_(_saved_positions)
    ...
    def propose(
        ...
        if prefill_batch_desc.cg_mode == CUDAGraphMode.FULL:
            ...
            # DEBUG probe: dump what the captured graph read/produced on replay.
            if self._probe_pos is not None and self._probe_n < 30:
                self._probe_n += 1
                import sys as _syd

                torch.xpu.synchronize()
                _h = [round(x, 2) for x in self._probe_hid.tolist()]
                _o = [round(x, 2) for x in self._probe_out.tolist()]
                print(
                    f"[SPEC_PROBE] n={self._probe_n} pos={self._probe_pos.tolist()} "
                    f"ids={self._probe_ids.tolist()} hid8={_h} out8={_o}",
                    file=_syd.stderr, flush=True,
                )
        else:
    ...
    def _run_model(
        ...
        with set_forward_context(
            ...
            # DEBUG probe (captured, copy_-only): record INPUTS read on replay.
            if self._probe_pos is not None:
                _pn = min(num_tokens, 8)
                self._probe_pos[:_pn].copy_(self.input_buffers.positions[:_pn])
                self._probe_ids[:_pn].copy_(self.input_buffers.input_ids[:_pn])
                self._probe_hid.copy_(self.hidden_states[0, :8])
            ...
            # DEBUG probe (captured, copy_-only): record OUTPUT produced on replay.
            if self._probe_pos is not None:
                _po = (
                    ret_hidden_states[0]
    ...
    def _prefill(
        ...
        # XPU FULL-capture fix: these writebacks set up the NEXT draft step and
        # are DEAD for num_speculative_steps == 1. But they get recorded into the
        # FULL cudagraph and, on every replay, clobber self.hidden_states /
        # self.input_buffers.positions (the conditioning + mask buffers that
        # propose() and the next replay read) -> stale conditioning -> ~36% accept
        # vs ~91% eager. Skip them when there is no next step (Python-time guard,
